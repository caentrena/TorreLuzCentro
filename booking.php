<?php include 'header.php';?>

    <section class="booking-page page-content">

        <div class="title-page"><p>Realizar pago</p></div>
        <div class="room-recordation">  
            <div class="room-image" style="background-image: url('assets/image/foto-slider2.jpg');"></div>
            <div class="room-information">
                <p class="name">Habitación Roquetas de mar</p>
                <div class="icons-container">
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 390.557 390.557" style="enable-background:new 0 0 390.557 390.557;" xml:space="preserve"> <g> <path style="fill:#231F20;" d="M389.772,248.296l-42.991-68.063V55.028l-17.5-17.5H61.275l-17.5,17.5v125.204L0.784,248.296 H389.772z M311.781,72.528v87.362c-22.553-5.834-61.514-13.028-116.503-13.028s-93.95,7.194-116.503,13.028V72.528H311.781z"/> <polygon style="fill:#231F20;" points="0,269.831 0,304.831 40.778,304.831 40.778,353.028 75.778,353.028 75.778,304.831 314.778,304.831 314.778,353.028 349.778,353.028 349.778,304.831 390.557,304.831 390.557,269.831   "/> <path style="fill:#231F20;" d="M181.075,128.653v-20.389c0-12.102-17.564-21.916-39.23-21.916s-39.229,9.814-39.229,21.916v30.604 c0,0,23.14-5.826,39.045-7.332C155.98,130.179,181.075,128.653,181.075,128.653z"/> <path style="fill:#231F20;" d="M287.941,138.868v-30.604c0-12.102-17.564-21.916-39.23-21.916s-39.23,9.814-39.23,21.916v20.389 c0,0,25.096,1.525,39.415,2.883C264.802,133.042,287.941,138.868,287.941,138.868z"/> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> </svg>
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"width="611.989px" height="611.988px" viewBox="0 0 611.989 611.988" style="enable-background:new 0 0 611.989 611.988;"xml:space="preserve"> <g> <g id="Wi-Fi"> <g> <path d="M305.994,417.769c-30.85,0-55.887,25.037-55.887,55.887s25.038,55.887,55.887,55.887s55.887-25.037,55.887-55.887 S336.843,417.769,305.994,417.769z M605.436,222.369C530.697,133.434,421.549,82.446,305.994,82.446 S81.309,133.434,6.551,222.369c-9.93,11.811-8.402,29.434,3.428,39.363c5.234,4.396,11.587,6.558,17.939,6.558 c7.973,0,15.891-3.391,21.423-9.967c64.084-76.248,157.639-119.989,256.652-119.989c99.013,0,192.568,43.741,256.651,119.971 c5.533,6.576,13.45,9.967,21.424,9.967c6.353,0,12.724-2.143,17.958-6.558C613.837,251.802,615.366,234.161,605.436,222.369z M305.994,194.22c-82.545,0-160.489,36.419-213.879,99.926c-9.929,11.811-8.402,29.434,3.428,39.363 c5.234,4.396,11.605,6.558,17.958,6.558c7.973,0,15.891-3.391,21.405-9.967c42.716-50.838,105.086-79.993,171.089-79.993 c66.003,0,128.372,29.155,171.107,79.993c5.533,6.595,13.45,9.967,21.404,9.967c6.353,0,12.724-2.143,17.959-6.558 c11.829-9.929,13.356-27.57,3.428-39.363C466.483,230.64,388.539,194.22,305.994,194.22z M305.994,305.994 c-49.553,0-96.331,21.852-128.335,59.948c-9.93,11.811-8.402,29.434,3.428,39.363c5.234,4.396,11.605,6.557,17.958,6.557 c7.973,0,15.891-3.39,21.405-9.966c21.368-25.429,52.552-40.016,85.544-40.016s64.177,14.587,85.544,40.016 c5.533,6.595,13.45,9.966,21.405,9.966c6.353,0,12.724-2.142,17.958-6.557c11.83-9.93,13.357-27.553,3.428-39.363 C402.324,327.846,355.546,305.994,305.994,305.994z"/> </g> </g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> </svg>
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 490 490" style="enable-background:new 0 0 490 490;" xml:space="preserve"> <g> <path d="M453.271,179.81L293.586,20.125C280.608,7.147,263.354,0,245,0s-35.608,7.147-48.586,20.125L36.729,179.81 c-12.919,12.919-16.602,31.431-9.609,48.311c5.227,12.619,15.314,21.663,27.718,25.426v167.742 c0,37.888,30.824,68.712,68.712,68.712h242.925c37.888,0,68.712-30.824,68.712-68.712V253.542 c12.387-3.765,22.471-12.809,27.695-25.422C469.873,211.241,466.19,192.729,453.271,179.81z M435.165,216.64 c-0.86,2.075-4.355,8.846-13.239,8.846c-9.23,0-16.74,7.51-16.74,16.74v179.062c0,21.346-17.366,38.712-38.712,38.712H123.549 c-21.346,0-38.712-17.366-38.712-38.712V242.226c0-9.23-7.51-16.74-16.763-16.74c-8.884,0-12.379-6.771-13.239-8.846 c-0.859-2.075-3.176-9.334,3.106-15.617L217.627,41.338C224.938,34.027,234.66,30,245,30s20.062,4.027,27.373,11.338 l159.686,159.685C438.341,207.306,436.024,214.565,435.165,216.64z"/> <path d="M372.657,212.73c-3.951-7.281-13.056-9.981-20.338-6.03l-15.047,8.166c1.665-6.294-0.92-13.181-6.785-16.695 c-7.106-4.258-16.319-1.948-20.576,5.156l-23.431,39.101l-20.645,11.203v-23.488l23.191-39.245 c4.214-7.132,1.85-16.331-5.283-20.545c-5.885-3.478-13.17-2.465-17.908,1.999v-17.119c0-8.284-6.716-15-15-15s-15,6.716-15,15 v17.12c-4.738-4.465-12.022-5.478-17.91-2c-7.132,4.215-9.496,13.413-5.282,20.545l23.192,39.245v23.488l-20.646-11.204 l-23.431-39.101c-4.258-7.105-13.469-9.416-20.576-5.156c-5.865,3.514-8.45,10.4-6.785,16.695L149.35,206.7 c-7.28-3.949-16.388-1.252-20.338,6.03c-3.951,7.281-1.252,16.387,6.029,20.338l15.047,8.165 c-6.184,2.035-10.549,7.955-10.299,14.787c0.296,8.092,6.949,14.452,14.98,14.452c0.185,0,0.372-0.003,0.559-0.01l45.552-1.667 l18.504,10.042l-18.504,10.041l-45.554-1.667c-8.277-0.312-15.235,6.163-15.538,14.442c-0.25,6.832,4.115,12.752,10.299,14.787 l-15.047,8.166c-7.281,3.951-9.98,13.057-6.029,20.338c2.717,5.008,7.873,7.849,13.197,7.849c2.416,0,4.867-0.585,7.141-1.819 l15.047-8.165c-1.664,6.294,0.921,13.18,6.785,16.694c2.414,1.447,5.072,2.136,7.695,2.136c5.099,0,10.07-2.601,12.882-7.292 l23.431-39.101l20.645-11.203v23.489l-23.191,39.243c-4.215,7.132-1.851,16.33,5.281,20.545c5.885,3.478,13.172,2.466,17.91-2 v17.121c0,8.284,6.716,15,15,15s15-6.716,15-15v-17.11c2.809,2.645,6.512,4.079,10.292,4.078c2.593,0,5.222-0.674,7.616-2.089 c7.133-4.214,9.497-13.413,5.283-20.545l-23.191-39.245v-23.488l20.645,11.203l23.43,39.101c2.813,4.692,7.782,7.292,12.882,7.292 c2.623,0,5.281-0.689,7.695-2.136c5.865-3.514,8.45-10.4,6.785-16.694l15.047,8.165c2.273,1.233,4.724,1.819,7.141,1.819 c5.323,0,10.48-2.841,13.197-7.849c3.951-7.281,1.252-16.387-6.029-20.338l-15.047-8.165c6.185-2.035,10.549-7.956,10.3-14.788 c-0.303-8.278-7.231-14.716-15.538-14.442l-45.554,1.667l-18.503-10.041l18.504-10.042l45.553,1.667 c0.188,0.007,0.373,0.01,0.559,0.01c8.031,0,14.685-6.36,14.98-14.452c0.25-6.832-4.115-12.753-10.3-14.788l15.047-8.165 C373.909,229.117,376.608,220.011,372.657,212.73z"/> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> <g> </g> </svg>
                </div>
                <p class="description"> viéndole don Quijote de aquella manera, con muestras de tanta tristeza, le dijo: Sábete, Sancho, que no es un hombre más que otro si no hace más que otro. Todas estas borrascas que nos suceden son señales de que presto ha de serenar el tiempo y han de sucedernos bien las cosas.</p>
            </div>
        </div>
        <div class="select-container">
            <div class="row">
                <input type="checkbox">
                <div class="option">
                    <p class="type">Solo alojamiento</p>
                    <p class="rooms">¡Solo quedan 3 habitaciones!</p>
                    <p class="price">30€</p>
                </div>
            </div>
            <div class="row">
                <input type="checkbox">
                <div class="option">
                    <p class="type">Media pension</p>
                    <p class="rooms">¡Solo queda 1 habitaciones!</p>
                    <p class="price">45€</p>
                </div>
            </div>
        </div>
        <p class="titular">Titular de la reserva</p>
        <div class="form-slider">
            <div class="form-slider-container medium-height">
                <div class="form-slider-page">
                    <div class="row-form">
                        <div class="label"><p>Nombre *</p></div>
                        <div class="input">
                            <div class="input "><input class="input-text" type="text"></div>
                        </div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>Apellido *</p></div>
                        <div class="input "><input class="input-text" type="text"></div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>DNI *</p></div>
                        <div class="input "><input class="input-text" type="text"></div>
                    </div>
                </div>
                <div class="form-slider-page">
                    <div class="row-form">
                        <div class="label"><p>Dirección *</p></div>
                        <div class="input">
                            <div class="input "><input class="input-text" type="text"></div>
                        </div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>Código postal *</p></div>
                        <div class="input">
                            <div class="input "><input class="input-text" type="text"></div>
                        </div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>País *</p></div>
                        <div class="input "><input class="input-text" type="text"></div>
                    </div>
                </div>
                <div class="form-slider-page">
                    <div class="row-form">
                        <div class="label"><p>Teléfono *</p></div>
                        <div class="input">
                            <div class="input "><input class="input-text" type="text"></div>
                        </div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>Email *</p></div>
                        <div class="input">
                            <div class="input "><input class="input-text" type="text"></div>
                        </div>
                    </div>
                    <div class="row-form">
                        <div class="label"><p>Observaciones *</p></div>
                        <div class="input "><textarea required class="input-text-area"></textarea></div>
                    </div>
                </div>
            </div>
            <div class="buttons">
                <div class="next-button button"><p>Siguiente</p></div>
                <div class="prev-button button"><p>Anterior</p></div>
            </div>  
        </div>
    </section>
    
<?php include 'footer.php';?> 